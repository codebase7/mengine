/*!
    Multiverse Engine Project 03/8/2016 Unit Tests Unit_Test_Data_Object_Insert_Replace_Overwrite_C_Str.cpp
...
/* Define test structure for C_Str function variants. */
typedef struct Unit_Test_Data_Object_From_C_Str_Funct_Test {
	int (*fp)(MSYS_DataObject_T * obj, const size_t offset, const char * src, const size_t srcLength);	/* Function pointer to C_Str function to test. */
...
} Unit_Test_Data_Object_From_C_Str_Funct_Test_T;

/* Define functions to test for Data_Object function variants. */
const Unit_Test_Data_Object_From_C_Str_Funct_Test_T Unit_Test_Data_Object_From_C_Str_Functions_To_Test[] = {
	{MSYS_DataObject_Insert_CString, "MSYS_DataObject_Insert_CString", true, true},
	{MSYS_DataObject_Insert_CString_No_Allocaton, "MSYS_DataObject_Insert_CString_No_Allocaton", true, false},
	{MSYS_DataObject_Replace_With_CString, "MSYS_DataObject_Replace_With_CString", false, false},
	{MSYS_DataObject_Overwrite_With_CString, "MSYS_DataObject_Overwrite_With_CString", false, true}
};

int Unit_Test_Data_Object_Insert_Replace_Overwrite_C_Str()
{
		/* Init retFromCall */
		...
		/* Begin test loop. */
		for (y = 0; y < (sizeof(Unit_Test_Data_Object_From_C_Str_Functions_To_Test) / sizeof(Unit_Test_Data_Object_From_C_Str_Funct_Test_T)); y++)
		{
			/* Output name of function we are testing. */
			std::cout << Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].functName << "() Tests: ";
			std::cout.flush();
			...
			/*
					...
					and the remaining data will be equal to s1 NOT including the first s2 number of bytes from s1.
			...
			*/
			...
			/* Run test function. */
			if (Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].fp(test_object_1_c_ptr, 0, s2.c_str(), s2.size()) !=
				((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ?
					/* Insert No Allocation function should return buffer too small here. All other functions should return success. */
					((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].canReallocate) ? (COMMON_ERROR_SUCCESS) : (COMMON_ERROR_MEMORY_BUFFER_TOO_SMALL)) :
			...
			if (test_object_1.size() != ((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ?
				((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].canReallocate) ? (s1.size() + s2.size()) : (s1.size())) :
				(s1.size())))
			{
					/* Invalid size after call to test function. */
					return -1014;
			}
			if (test_object_1.get_Capacity() != ((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ?
				((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].canReallocate) ? (s1.size() + s2.size()) : (s1.size())) :
				(s1.size())))
			{
			...
			/* Make sure that test_object_1 actually contains the correct data. */
			for (size_t x = 0; x < ((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ?
				((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].canReallocate) ? (s1.size() + s2.size()) : (s1.size())) :
				(s1.size())); x++)
			{
					/* If the current function we are testing is Insert No Allocate, then the data will be identical to s1. */
					if ((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) &&
						(!(Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].canReallocate)))
					{
							if (p1[x] != p2[x])
							{
									/* DataProcess::Data_Object (Manual compare) Invalid data in the data_object. */
					...
					else
					{
						...
						{
								if (p1[x] != p2[((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ?
									((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].canReallocate) ? (x - (s2.size())) : (x)) :
									(x))])
								{
										/* DataProcess::Data_Object (Manual compare) Invalid data in the data_object. */
										return -1004;
			...
			/*
					...
					This test checks a C_Str function for MSYS_DataObjects. (Reallocate)

					This test attempts to store s3 in test_object_1, then run the test function with an offset of
					position 4 and the data of s2.
			...
			*/
			...
			/* Run test function. */
			if (Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].fp(test_object_1_c_ptr, 4, s2.c_str(), s2.size()) !=
			...
			*/
			if (test_object_1.size() !=
				((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].canReallocate) ?
					((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ? (s3.size() + s2.size()) :
						(s3.size() + s2.size() - (4 + 1))) :
					(s3.size())))
			{
					/* Invalid size after call to test function. */
			...
			for (size_t x = 0; x < test_object_1.size(); x++)
			{
					...
					if (p1[x] !=
				((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].canReallocate) ? ((x < 4) ? (p2[x]) : ((x < (4 + s2.size())) ? (p3[(x - 4)]) : (p2[(x - s2.size())]))) :
				(p2[x])))
					{
							/* DataProcess::Data_Object (Manual compare) Invalid data in the data_object. */
							return -1004;
			...
			/*
					...
					This test checks a C_Str function for MSYS_DataObjects. (Non-Reallocate)
			...
			*/
			...
			test_object_1.reserve(((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ? (s3.size() + s2.size()) : (s3.size())));
			...
			if (test_object_1.get_Capacity() != ((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ? (s3.size() + s2.size()) : (s3.size())))
			{
			...
			*/
			...
			/* Run test function. */
			if (Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].fp(test_object_1_c_ptr, 0, s2.c_str(), s2.size()) != COMMON_ERROR_SUCCESS)
			{
					/* Test function failed. */
					return -1020;
			}
			...
			if (test_object_1.size() != ((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ? (s3.size() + s2.size()) : (s3.size())))
			{
					/* DataProcess::Data_Object::operator= Invalid size after call to DataProcess::Data_Object::operator= std::string. */
					return -1014;
			}
			...
			for (size_t x = 0; x < test_object_1.size(); x++)
			{
					...
					{
							if (p1[x] != p2[((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ? (x - s2.size()) : (x))])
							{
									/* DataProcess::Data_Object (Manual compare) Invalid data in the data_object. */
									return -1004;
							}
			...
			*/
			...
			test_object_1.reserve(((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ? (s1.size() + s2.size()) : (s1.size())));
			...
			if (test_object_1.get_Capacity() != ((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ? (s1.size() + s2.size()) : (s1.size())))
			{
					/* DataProcess::Data_Object::reserve() Invalid capacity after call to reserve(). */
					return -1007;
			}
			...
			*/
			...
			/* Run test function. */
			if (Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].fp(test_object_1_c_ptr, 3, s2.c_str(), s2.size()) != COMMON_ERROR_SUCCESS)
			{
					/* Test function failed. */
					return -1020;
			}
			 ...
			 */
			if (test_object_1.size() != ((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ? (s1.size() + s2.size()) : (s1.size())))
			{
					/* DataProcess::Data_Object::operator= Invalid size after call to DataProcess::Data_Object::operator= std::string. */
					return -1014;
			}
			...
			for (size_t x = 0; x < test_object_1.size(); x++)
			{
					...
					*/
					if (p1[x] != ((x < 3) ? (p2[x]) :
						((x < (3 + s2.size())) ? (p3[(x - 3)]) :
						 ((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].isInsert) ? (p2[(x - s2.size())]) :
						  (p2[x])))))
					{
							/* DataProcess::Data_Object (Manual compare) Invalid data in the data_object. */
							return -1004;
			 ...
			 */
			if ((Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].fp(NULL, 3, s1.c_str(), s1.size()) != COMMON_ERROR_INVALID_ARGUMENT) ||
				(Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].fp(test_object_1_c_ptr, 3, NULL, 33) != COMMON_ERROR_INVALID_ARGUMENT))
			{
			...
			/*
				...
				Should return COMMON_ERROR_INVALID_ARGUMENT.
			 */
			if (Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].fp(&badPrivPtr, 3, s1.c_str(), s1.size()) != COMMON_ERROR_INVALID_ARGUMENT)
			{
					/* Test function failure. Should have returned COMMON_ERROR_INVALID_ARGUMENT. */
					return -1020;
			}
			 ...
			 */
			if (Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].fp(&badData, 0, s1.c_str(), s1.size()) != COMMON_ERROR_DATA_CORRUPTION)
			{
					/* Test function failure. Should have returned COMMON_ERROR_DATA_CORRUPTION. */
					return -1020;
			}
			if (Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].fp(&badLength, 0, s1.c_str(), s2.size()) != COMMON_ERROR_DATA_CORRUPTION)
			{
					/* Test function failure. Should have returned COMMON_ERROR_DATA_CORRUPTION. */
					return -1020;
			}
			if (Unit_Test_Data_Object_From_C_Str_Functions_To_Test[y].fp(&badCapacity, 0, s1.c_str(), s2.size()) != COMMON_ERROR_DATA_CORRUPTION)
			{
					/* Test function failure. Should have returned COMMON_ERROR_DATA_CORRUPTION. */
					return -1020;
			}
