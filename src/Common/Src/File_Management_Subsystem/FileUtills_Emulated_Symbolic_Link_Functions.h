...
/* Define size of the byte count field. */
#define MSYS_EMU_SYMLINK_BYTE_COUNT_FIELD_SIZE 4
 ...
 *
 * 	Offset (0 + sizeof(<Magic string>)): unsigned 32-bit integer (4 bytes, big endian) size of the length value. (I.e. sizeof(size_t)).
 * 
 ...
 *
 * 	Offset (0 + sizeof(<Magic string>) + 4 + (size of the length value) + <length of the link>): End of the link, and end of file.
 */
...
{	
	 ...
	 *
	 * 	If successful this function will return COMMON_ERROR_SUCCESS, and the emulated symbolic link file
	 * 	will be present at the given location on the filesystem.
	 *
	 * 	If an error occurs, the appropriate Common name-space error code will be returned. In addition, the
	 ...
	 */
	int Create_MSYS_Emulated_Symbolic_Link(const char * linkDestionation, const size_t linkDestionationSize,
										   const char * pathToLinkFile, const size_t pathToLinkFileSize);
	 ...
	 *
	 * 	NOTE: This function is called automatically by FileUtills::ResolvePath() if symbolic link resolution is
	 * 	enabled by the caller (it is by default), and an emulated link file is found during path resolution.
	 * 	As such it's not necessary to call this function directly before FileUtills::ResolvePath().
	 *
	 ...
	 * 	 assumption.))
	 *
	 * 	If successful, the given link argument string will have it's contents replaced with the read link
	 * 	string, and COMMON_ERROR_SUCCESS will be returned.
	 *
	 * 	If an error occurs then the given link argument string will be blank / empty, and the appropriate
	 * 	Common name-space error code will be returned.
	 */
	int Read_MSYS_Emulated_Symbolic_Link(char ** retStr, size_t * retStrSize, const char * pathToLinkFile, const size_t pathToLinkFileSize);

	/*!
	 * 	int FileUtills::Update_MSYS_Emulated_Symbolic_Link(const char * linkDestionation, const size_t linkDestionationSize,
	 *													   const char * pathToLinkFile, const size_t pathToLinkFileSize)
	 *
	 ...
	 *
	 * 	If successful, the given emulated link file will contain the contents of the given linkDestionation argument string, and
	 ...
	 * 	filesystem however, as such it will need to be deleted manually.)
	 */
	int Update_MSYS_Emulated_Symbolic_Link(const char * linkDestionation, const size_t linkDestionationSize,
										   const char * pathToLinkFile, const size_t pathToLinkFileSize);
};

#endif	/* FILEUTILLS_EMULATED_SYMBOLIC_LINK_FUNCTIONS_H */
